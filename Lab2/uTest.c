...
static char* test_push()
{
    DLList * myList = createDLList();
    push(myList, 1);
    mu_assert("error - (push) incorrect", 2 == push(myList, 1));
    free(myList);
    return 0;
}
...
static char* test_pop()
{
    ...
    pop(myList);
    mu_assert("error - (pop) incorrect after pop", 0==pop(myList));
    free(myList);
    return 0;
}
...
static char* test_atEnd()
{
    ...
    first(myList);
    mu_assert("error - (atEnd) incorrect after first", !atEnd(myList));

    next(myList);
    mu_assert("error - (atEnd) incorrect after next", atEnd(myList));
...
static char* test_deleteCurrent()
{
    ...
    deleteCurrent(myList);
    mu_assert("error - (deleteCurrent) incorrect after deleteCurrent", 1==getCurrent(myList));
    free(myList);
    return 0;
